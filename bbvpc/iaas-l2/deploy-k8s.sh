...
declare -r -a _k8s_worker=(${_ip_prefix}.21 ${_ip_prefix}.22 ${_ip_prefix}.23)
declare -r -a _k8s_node=(${_k8s_master[*]} ${_k8s_worker[*]})
declare -r -a _k8s_master_hostname=(`for i in "${!_k8s_master[@]}"; do echo "k8s-master-$[$i+1] ";done`)
declare -r -a _k8s_worker_hostname=(`for i in "${!_k8s_worker[@]}"; do echo "k8s-worker-$[$i+1] ";done`)
declare -r -a _k8s_node_hostname=(${_k8s_master_hostname[*]} ${_k8s_worker_hostname[*]})
# 定义浮动IP地址
declare -r    _k8s_virtual_ip="${_ip_prefix}.10"
# 获取外部浮动IP地址
#declare -r    _k8s_virtual_fip=`echo "127.0.0.1"`
...
function init_hosts(){
    ...
    for i in "${!_k8s_node[@]}"; do
        if grep -w "${_k8s_node[$i]}" "${_file_hosts}" > /dev/null 2>&1; then
            eval "sed -i '${_host_file}'-e '/\<${_k8s_node[$i]}\>/s/$/ k8s-node-$[$i+1]/'"
        else
            echo "${_k8s_node[$i]} k8s-node-$[$i+1]" >> "${_file_hosts}"
        fi
    done

    for i in "${!_k8s_master[@]}"; do
        if grep -w "${_k8s_master[$i]}" "${_file_hosts}" > /dev/null 2>&1; then
            eval "sed -i '${_file_hosts}' -e '/\<${_k8s_master[$i]}\>/s/$/ k8s-master-$[$i+1]/'"
        else
            echo "${_k8s_master[$i]} k8s-master-$[$i+1]" >> "${_file_hosts}"
        fi
    done

    for i in "${!_k8s_worker[@]}"; do
        if grep -w "${_k8s_worker[$i]}" "${_file_hosts}" > /dev/null 2>&1; then
            eval "sed -i '${_file_hosts}' -e '/\<${_k8s_worker[$i]}\>/s/$/ k8s-worker-$[$i+1]/'"
        else
            echo "${_k8s_worker[$i]} k8s-worder-$[$i+1]" >> "${_file_hosts}"
        fi
    done

    echo "${_k8s_virtual_ip} k8s-master ${K8S_CLUSTER_NAME} ${K8S_CLUSTER_NAME}.yealink.top" >> "${_file_hosts}"
    cat "${_file_hosts}"
}
...
docker run --network host --rm plndr/kube-vip:v0.3.8 manifest pod \
    ...
    --arp \
    --leaderElection \
| sed -e 's|ghcr.io/kube-vip|plndr|g' \
